...
@SilentLayerActor public class OpenAIService: Sendable {
    ...
    public func createResponse(
        ...
        guard !input.isEmpty else {
            throw SilentLayerError.invalidConfiguration("Input cannot be empty")
        }
        guard (0...2).contains(temperature) else {
            throw SilentLayerError.invalidConfiguration("Temperature must be between 0 and 2")
        }
    ...
    public func generateImage(
        ...
        guard !prompt.isEmpty else {
            throw SilentLayerError.invalidConfiguration("Prompt cannot be empty")
        }
        guard (1...10).contains(n) else {
            throw SilentLayerError.invalidConfiguration("Number of images must be between 1 and 10")
        }
